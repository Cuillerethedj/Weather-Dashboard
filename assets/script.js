...
function getAPI() // Calls our API to obtain the weather information that we need.
{
        ...
        .then(function(data) // We take that data and use it inside this function to start outputting our information
    {
            ...
            .then(function(data2) // We take that data and use it inside this function to start outputting our information.
        {
            ...
            $('.myCardTemp').each(function(i) {
                $(this).text("Temp: " + data2.daily[i].temp.day);
            }); // Runs a for EACH loop that displays each days temperature (after the first curly bracket you can start a new line to make it easier to read)
            $('.myCardPhoto').each(function(i) {
                $(this).prepend('<img src = "http://openweathermap.org/img/wn/' + data2.daily[i].weather[0].icon + '@2x.png" />');
            }); // Runs a for EACH loop that displays each days weather icon  (after the first curly bracket you can start a new line to make it easier to read)
            $('.myCardWind').each(function(i) {
                $(this).text("Wind: " + data2.daily[i].wind_speed + "MPH");
            }); // runs a for EACH loop that displays each days wind speed (after the first curly bracket you can start a new line to make it easier to read)
            $('.myCardHumidity').each(function(i) {
                $(this).text("Humidity: " + data2.daily[i].humidity + '%');
            }); // runs a for EACH loop that displays each days humidity  (after the first curly bracket you can start a new line to make it easier to read)
            $('.myCardDate').each(function(i) {
                $(this).text(moment().add(1 + i, 'days').format("MM/DD/YYYY"));
            }); // runs a for EACH loop that displays each days date  (after the first curly bracket you can start a new line to make it easier to read)
        })
    })
}
...
function check() // A function that checks if we have any previous cities inside of our local storage, if we do not we simply return our saveCity array and keep moving
{
    ...
    } // Returns our cities from inside the local storage
    else {
        return saveCity;
    } // If there is nothing in local storage we simply return our array back
}
...
function save() // Saves our new city that the user has just inputted and puts it inside of the array with the other cities
{
    ...
    } // Sets our new array inside of local storage
}
